...
\noindent Assim, $T$ pode ser calculado conforme equação em (\ref{eq:s17}):

\begin{equation}
\label{eq:s17}
 T = \lfloor \min_i\{{\textrm{UD}_i / \textrm{p}_i}\} + \min\{ \min_i\{ (\textrm{UO}_i + \textrm{UI}_i + S_i) / \textrm{p}_i\}, (\textrm{O} + \textrm{I} + \sum_i S_i) / \sum_i \textrm{p}_i \}\rfloor
\end{equation}
...
After relaxation of the problem, 
