...
\begin{algorithmic}
	...
	\State $I_{1} \gets \textrm{I} - \sum\{\textrm{PO}_{i1}\}$
	\State $O \gets \textrm{O}$
	
	\For{all $i$}
		\State $D_{i1} \gets \min \{\textrm{UD}_{i1}, \textrm{PO}_{i1}\}$
		\State $UI_{i1} \gets \textrm{UI}_i - \textrm{PO}_{i1} + D_{i1}$
		\State $I_{1} \gets I_{1} + D_{i1}$
		
		\If{$UI_{i1} < 0$}
			\State $O_{i1} \gets - UI_{i1}$
			\State $UO_i \gets \textrm{UO}_i - O_{i1}$
			\State $O \gets O - O_{i1}$
			\State $I_{1} \gets I_{1} + O_{i1}$
			\State $UI_{i1} \gets 0$
			\If{$UO_i < 0 \textbf{ or } O < 0$}
			\State \textbf{Return:} planning is not feasible
			\EndIf
		\EndIf
	\EndFor
	...
	\For{all $d>1$}
		...
		\State $I_{d} \gets  I_{d-1} - \sum\{\textrm{PO}_{id}\}$
		...
		\For{all $i$}
			...
			\State $I_{d} \gets I_{d} + D_{id}$
		
			\If{$UI_{id} < 0$}
				\State $O_{i1} = - UI_{i1}$
				\State $UO_i = UO_i - O_{id}$
...
\begin{algorithmic}
	...
	\For{all $d$}
		\If{$I_{d} < 0$}
			\For{all $i$}
				\If{$UO_i \geq |I_{d}|$}
					\State $O_{id} \gets O_{id} - I_{d}$
					\State $UO_i \gets UO_i + I_{d}$
					\State $O \gets O + I_{d}$
					\For{all $k \geq d$}
						\State $UI_{ik} \gets UI_{ik} - I_{d}$
						\State $I_{k} \gets I_{k} - I_{d}$
					\EndFor
					\State break;
				\Else 
					\State $O_{id} \gets O_{id} + UO_i$
					\State $UO_i \gets 0$
					\State $O \gets O - UO_i$
					\For{all $k \geq d$}
						\State $UI_{ik} \gets UI_{ik} + UO_i$
						\State $I_{k} \gets I_{k} + UO_i$
					\EndFor
				\EndIf
			\EndFor
		\EndIf
