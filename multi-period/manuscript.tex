...
passo 2: Calcule os valores $D_{id}^+$, $D_{id}^-, \forall i, d$, usando as equações em (\ref{eq:sm02}) e (\ref{eq:sm03}):

\begin{equation}
\label{eq:sm02}
D_{id}^+ = \sum_{k=1}^{d}{\textrm{UD}_{ik}} - \sum_{k=1}^{d-1}{D_{ik}}  \quad \forall i, \forall d>1
\end{equation}

\begin{equation}
\label{eq:sm03}
D_{id}^- = \sum_{k=2}^{d}{\textrm{UD}_{ik}} - \sum_{k=1}^{d-1}{D_{ik}}  \quad \forall i, \forall d>1
\end{equation}

Calcule $D_{i1}$ usando as equações em (\ref{eq:sm04}):
...
queremos encontrar $D_{id}$, tal que a equação em (\ref{eq:sm04}) seja respeitada:

\begin{equation}
\label{eq:sm04}
D_{id} = \min {x \in \{D_{id}^-, D_{id}^+\}} \textrm{ tal que } UI_{id} \geq 0
\end{equation}
...
Inicialmente desconsidere a produção a ser planejada fazendo $P_i = 0, \forall i$, e 

Tais valores podem ser calculados da seguinte forma:

- distribua a produção previamente planejada para atender à demanda seguindo as restrições em (9) e para as duas equações derivadas de (10):

as restrições em (14), (15) e (16) foram criadas porque queremos permitir que a demanda referênte ao dia atual seja atendida pela produção planejada para o dia atual e não por produções futuras.
