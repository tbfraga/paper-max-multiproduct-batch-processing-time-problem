...
%%Research highlights
\begin{highlights}
\item this paper presents a brief bibliographic review on multi-product batch problems;
\item presents a new combinatorial optimization problem named Multi-product Batch Processing Time Maximization (MPBPTM) problem;
\item presents a linear integer programming model for the MPBPTM problem;
\item proposes a polynomial time analytical method for solving the MPBPTM problem.
\end{highlights}
...
To test the developed model and analytical solution method, we created a solver in C++ (https://github.com/tbfraga/project-combinatorial-optimization-problems/tree/main/combinatorial-optimization-solver) and a solver in LINGO (https://github.com/tbfraga/project-combinatorial-optimization-problems/tree/main/lingo-solvers/MPBPTMP). The tests were performed on a notebook with an Intel i7 processor. We tested the solvers developed for the benchmarks presented below (and available on github https://github.com/tbfraga/project-combinatorial-optimization-problems/tree/main/lingo-solvers/MPBPTMP/benchmarcks).
...
It is important to point out that the LINGO solver was developed with the purpose of validating the results found by the proposed analytical method. As the analytical method is a polynomial method, we did not intend to compare machine processing times, however it is possible to verify that both solvers are capable of finding optimal solutions for the proposed benchmarks very quickly, even for very large problems.
