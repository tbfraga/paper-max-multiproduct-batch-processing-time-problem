...
Observe that $T$ is not affected by the way production is distributed betwing demand, outlets and factory inventory. However, after determining a solution, the values of $D_i$, $O_i$ and $I_i$, $\forall i$, can be found by taking a solution from the undetermined system of inequalities:
...
It seems natural that every industry has as a priority to meet demand and that it is preferable to send the remaining production to outlets rather then stock it in the factory. Therefore, we can determine the values of $D_i$, $O_i$ and $I_i$, $\forall i$, simply by defining these priorities for distribution. The following three algorithms can be used to find a complete solution:
...
To test the developed model and analytical solution method, we created a solver in C++ and a solver in LINGO (https://github.com/blinded). The tests were performed on a notebook with an Intel i7 processor. We tested the solvers developed for the benchmarks presented on Tables \ref{tab:MBPTMP001}, \ref{tab:MBPTMP002} and \ref{tab:MBPTMP003}, and for randon benchmarks generated by the following functions: \\
...
Blinded, Project administration, Supervision, Software, Methodology, Validation, Formal analysis, Writing – original draft, Writing – review \& editing. Blinded: Data curation. Blinded: Data curation.
